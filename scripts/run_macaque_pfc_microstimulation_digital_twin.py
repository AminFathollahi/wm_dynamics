#!/usr/bin/env python3
"""Digital-twin input-LDS depth move.

WHY: the existing macaque PFC microstimulation causal arm (run_macaque_pfc_microstimulation_pipeline.py) only
...
PI GUARDRAILS (mandatory):
  (i)   NO-CIRCULARITY: A (and C) are fit from CONTROL / no-stim epochs
        ...
        space (C), EXACTLY as run_macaque_pfc_microstimulation_pipeline.build_session_features
...
same call run_macaque_pfc_microstimulation_pipeline.py already makes for the CATE-vs-alignment
...
Run:
    conda run -n wm_dynamics python scripts/run_macaque_pfc_microstimulation_digital_twin.py
"""
...
import run_macaque_pfc_microstimulation_pipeline as macaque_pfc_microstimulation  # noqa: E402  (reuse SESSIONS, loaders, N_BINS, PRE_S, BIN_S, N_PC, DMD_RANK)
...
def _fit_twin_for_session(prefix: str) -> dict | None:
    ...
    # zero pre-onset -- reconstructable per-step stim timing (the feasibility
    # gate this arm exists to pass). Latent displacement magnitude = the
    ...
    # aipw_pseudo_outcome call run_macaque_pfc_microstimulation_pipeline.py already makes for the
...
def main():
    ...
    if not per_session or len(all_twin) == 0:
        out = {
            "status": "infeasible",
            "reason": "No macaque PFC microstimulation session yielded a usable input U (amp/onset structure "
