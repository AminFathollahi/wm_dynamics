...
Outputs (per session):
  ...
  results/dandi001187_summary.json
Updates:
  results/all_statistics.json — "dandi001187_ctg" and "content_ctg_pooled" keys

Run:
    conda run -n wm_dynamics python scripts/run_001187_pipeline.py
"""
...
def _process_session(fp: str):
    ...
    with threadpool_limits(limits=1):
        with h5py.File(fp, "r") as f:
            if "WM_trials" not in f.get("intervals", {}):
                return None
        ...
        np.savez_compressed(
            RESULTS / f"dandi001187_geometry_{key}.npz",
        ...
        if ctg_res is not None:
            np.savez_compressed(
                RESULTS / f"dandi001187_ctg_{key}.npz",
...
def main():
    ...
    print(f"\nFDR (BH), 001187 load1v3 CTG: {fdr['n_reject']}/{len(p_vals)} sessions survive q<0.05")
    ...
    print(f"Correct-vs-error drift (001187, pooled N={len(drift_arr)}): "
          f"beta={lme_drift['beta']:.4f}, p={lme_drift['p_value']:.4f}")

    stats_path = RESULTS / "all_statistics.json"
    with open(stats_path) as f:
        stats = json.load(f)
    stats["dandi001187_ctg"] = summary
    stats["dandi001187_correct_error_drift"] = {
        "beta": lme_drift["beta"], "p_value": lme_drift["p_value"], "n_trials": int(len(drift_arr)),
    }

    rushi_content = stats.get("dandi000469_ctg", {})
    p_pool = [v["content_ctg"]["p_value"] for v in summary.values()
              if v.get("content_ctg") is not None] + [
        v["content_ctg"]["p_value"] for v in rushi_content.values()
        if v.get("content_ctg") is not None
    ]
    if len(p_pool) >= 2:
        ...
        print(f"\nPooled item-identity CTG (000469 + 001187, N={len(p_pool)} sessions, "
