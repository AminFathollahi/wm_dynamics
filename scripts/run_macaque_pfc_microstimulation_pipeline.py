#!/usr/bin/env python3
"""Macaque dlPFC delay-period microstimulation, 2025 J Neurosci release.
...
subspace) and the macaque PFC microstimulation-language dynamic-vs-stable replication (WP-CAUSAL-5)
are deferred to Phase 2 once the gate result is in.

Outputs:
  results/causal_macaque_pfc_microstimulation.json -- per-session features + the pooled CATE-vs-
    alignment slope test (the gate).
  results/all_statistics.json -- "causal_macaque_pfc_microstimulation" key.

Run:
    conda run -n wm_dynamics python scripts/run_macaque_pfc_microstimulation_pipeline.py
"""
...
from project_config import data_asset_path, data_root, dataset_path, executable, project_path
...
from control import (controllability_gramian, average_controllability, modal_controllability,
                    invariant_subspace_basis, subspace_alignment, canonicalize_eigenvector_phase)
...
DATA = (
    Path(_DATA_ROOT) / _DATA_CONFIG["datasets"]["macaque_pfc_microstimulation"]["local_path"]
    if _DATA_ROOT
    else ROOT / "__WM_DYNAMICS_DATA_ROOT_NOT_SET__"
)
...
DMD_RANK = 7   # A reported choice, not a data-selected optimum: the cross-validated one-step R^2
               ...
               # r in {5,6,7,8} robustness lives in the same file's per-r benchmark_slope/
               # benchmark_p (run_dmd_rank_selection.py's benchmark_slope_at_rank, which flips
               # this constant per-rank on this module).
GRAMIAN_HORIZON = 20
...
def _load_h5py_session(mat_path: Path, neural_field: str = "spikerate") -> dict | None:
    ...
    with f:
        ...
        return {
            ...
            "trialsequence_present": "trialsequence" in params,
            "n_trialsequence": (
                int(np.asarray(params["trialsequence"]).size)
                if "trialsequence" in params else 0
            ),
        }


def _load_scipy_session(mat_path: Path, neural_field: str = "spikerate") -> dict | None:
    ...
    n_stim_cond, n_angle = d[neural_field].shape[0], d[neural_field].shape[1]
...
def _shorted_channel_keep_mask(prefix: str, channel_ids: np.ndarray) -> np.ndarray:
    """Boolean keep-mask, aligned to `channel_ids`, for electrically shorted
    contacts (data/shortedchan/{prefix}_shorted.mat).

    Two file layouts, one per monkey: Wa gives `channelsKeep`, a 1-based
    electrode-ID whitelist (channel_ids run 1..96, so a direct isin applies).
    Sa gives `shortchmaskrec`/`shortchmaskstim`, two 96-length boolean masks
    for its two Utah arrays (1 = shorted); Sa's channel_ids run
    [1..96, 257..352] in that same array order, so the masks concatenate
    positionally with no re-indexing.
    """
    shorted_path = DATA / "shortedchan" / f"{prefix}_shorted.mat"
    if not shorted_path.exists():
        return np.ones(len(channel_ids), dtype=bool)
    import scipy.io as sio

    m = sio.loadmat(str(shorted_path), simplify_cells=True)
    if "channelsKeep" in m:
        keep_ids = set(np.atleast_1d(m["channelsKeep"]).astype(int).tolist())
        return np.array([int(c) in keep_ids for c in channel_ids])
    rec_mask = np.asarray(m["shortchmaskrec"]).ravel().astype(bool)
    stim_mask = np.asarray(m["shortchmaskstim"]).ravel().astype(bool)
    shorted = np.concatenate([rec_mask, stim_mask])
    if len(shorted) != len(channel_ids):
        raise ValueError(f"{prefix}: shorted-channel mask length {len(shorted)} != "
                        f"channel_ids length {len(channel_ids)}")
    return ~shorted


def _apply_channel_filter(session: dict, prefix: str) -> dict:
    keep = _shorted_channel_keep_mask(prefix, session["channel_ids"])
    if keep.all():
        return session
    for trial in session["trials"]:
        trial["spikerate"] = trial["spikerate"][:, keep]
    session["channel_ids"] = session["channel_ids"][keep]
    session["n_channels_dropped_shorted"] = int((~keep).sum())
    return session


def load_macaque_pfc_microstimulation_session(prefix: str, correct: bool, neural_field: str = "spikerate") -> dict | None:
    ...
    Electrically shorted channels (data/shortedchan/) are dropped here once,
    so every caller inherits the exclusion.
    """
    import h5py
    ...
    try:
        ...
        is_h5 = False
    session = (
        _load_h5py_session(mat_path, neural_field=neural_field)
        if is_h5 else _load_scipy_session(mat_path, neural_field=neural_field)
    )
    if session is None:
        return None
    return _apply_channel_filter(session, prefix)
...
def build_session_features(prefix: str, structural_ctrl: dict | None = None,
    ...
    the inputs needed to re-score alignment against a direction other than
    v_star without re-fitting PCA/DMD (see run_vstar_phase_sensitivity.py)."""
    ...
    # w1: the top RIGHT SINGULAR vector of the SAME control-epoch-fit A --
    # the one-step maximal-gain input direction for a non-normal plant (see
    # scripts/run_amplification_check.py; median cos(v*,w1)=0.36 across
    # sessions -- the two directions genuinely differ here, so both are
    # scored as competing causal targets below).
    ...
    v_stable = canonicalize_eigenvector_phase(vecs[:, stable_order[0]])

    # S_m: the real invariant subspace of A's m=2/3 largest-|lambda| modes
    # (src/control.invariant_subspace_basis -- a complex-conjugate pair is
    # ONE mode, 2 real dims, not two). Strict generalization of v_star/M1
    # (equal at m=1 on a real mode).
    subspace_s_m2 = invariant_subspace_basis(A, 2)
    subspace_s_m3 = invariant_subspace_basis(A, 3)
    basis_s_m2, basis_s_m3 = subspace_s_m2.basis, subspace_s_m3.basis
    subspace_basis_status = {
        "m2": {"status": subspace_s_m2.status, "dim": subspace_s_m2.dim, "notes": subspace_s_m2.notes},
        "m3": {"status": subspace_s_m3.status, "dim": subspace_s_m3.dim, "notes": subspace_s_m3.notes},
    }

    if geometry_out is not None:
        geometry_out["A"] = A
        geometry_out["eigs"] = eigs
        geometry_out["vecs"] = vecs
        geometry_out["cond_B_hat"] = {}
        geometry_out["subspace_basis_status"] = subspace_basis_status

    # M4 no-structure null floor: fixed-seed random unit directions in the
    # SAME latent space, independent of the fitted dynamics.
    rand_rng = np.random.default_rng(stable_seed(f"macaque_pfc_microstimulation_random_dirs_{prefix}"))
    random_dirs = rand_rng.standard_normal((N_RANDOM_DIRS, k))
    random_dirs /= np.linalg.norm(random_dirs, axis=1, keepdims=True) + 1e-12

    # Channel-space random-direction null: a FAIRER no-structure floor than
    # M4's isotropic-LATENT random_dirs, since the actual delivered stim
    # direction is always a one-hot CHANNEL indicator pushed through V
    # (never an isotropic latent draw); V's rows are not uniform across
    # channels, so this is a genuinely different null.
    chan_rand_rng = np.random.default_rng(stable_seed(f"macaque_pfc_microstimulation_channel_random_dirs_{prefix}"))
    chan_random_idx = chan_rand_rng.integers(0, C, size=N_RANDOM_DIRS)
    chan_random_dirs_lat = np.zeros((N_RANDOM_DIRS, k))
    for _i, _ci in enumerate(chan_random_idx):
        _e = np.zeros(C)
        _e[_ci] = 1.0
        _v = V.T @ _e
        chan_random_dirs_lat[_i] = _v / (np.linalg.norm(_v) + 1e-12)

    # 1b. WP-CAUSAL-5: dynamic-vs-stable subspace split (this release's
    ...
    for c in range(n_cond):
        ...
        if len(idx) != len(chan_ids):
            # Require every stimulation electrode of this condition to survive
            # the shorted-channel exclusion -- a partial B built from only
            # some of them (e.g. one member of a bipolar pair) misrepresents
            # the delivered input and can make distinct conditions collapse.
            continue
        B_chan = np.zeros((C, 1))
        ...
        if geometry_out is not None:
            geometry_out["cond_B_hat"][c] = B_hat_unit
        ...
        # M7: alignment to w1, the propagator's top singular vector -- the
        # amplification-reframe competitor to v_star.
        amplification_alignment = float(np.abs(B_hat_unit @ w1))

        # M8: within/outside-manifold fractions of the raw channel-space stim
        # direction B_chan (== e_hat in run_manifold_constraint.py, same P_k),
        # NOT B_lat (which is
        ...
        # Subspace-alignment arms (m=2/3), tested against vstar_alignment (m=1).
        align_s_m2 = subspace_alignment(basis_s_m2, B_hat_unit)
        align_s_m3 = subspace_alignment(basis_s_m3, B_hat_unit)

        # Controls: top-m PCA-VARIANCE-ranked alignment
        # (dynamics-free -- no operator A at all, just how much of B_hat_unit
        # loads onto the leading m variance-ranked latent axes e_1..e_m, which
        # ARE the first m columns of V by pca_decompose's own ordering) and
        # the channel-space random-direction null (chan_random_dirs_lat, built
        # once per session above).
        topm_pca_alignment_m2 = float(np.linalg.norm(B_hat_unit[:2]))
        topm_pca_alignment_m3 = float(np.linalg.norm(B_hat_unit[:min(3, k)]))
        channel_random_alignment = float(np.mean(np.abs(chan_random_dirs_lat @ B_hat_unit)))
        ...
        cond_features[c] = {
            ...
            "align_s_m2": align_s_m2,
            "align_s_m3": align_s_m3,
            "topm_pca_alignment_m2": topm_pca_alignment_m2,
            "topm_pca_alignment_m3": topm_pca_alignment_m3,
            "channel_random_alignment": channel_random_alignment,
        }
    ...
    modifier_keys = [k for k in ("gramian_trace", "stable_alignment", "random_alignment",
                                 ...
                                 "align_s_m2", "align_s_m3", "topm_pca_alignment_m2",
    ...
    for c, feat in cond_features.items():
        ...
        for _, y, angle_idx in stim_all:
            rows.append({"y": y, "t": 1, "modifier": feat["alignment_to_vstar"], "cond": c,
                         "propensity": propensity, "angle_idx": angle_idx, **extra})
        for _, y, angle_idx in ctrl_all:
            rows.append({"y": y, "t": 0, "modifier": feat["alignment_to_vstar"], "cond": c,
                         "propensity": propensity, "angle_idx": angle_idx, **extra})
...
def main():
    ...
    if not all_rows:
        print("No usable macaque PFC microstimulation sessions -- stopping without a gate result.")
        return
    ...
    rng = np.random.default_rng(stable_seed("macaque_pfc_microstimulation_causal_gate"))
    ...
    # Trial-resolution dissociation arm. v_star itself is already fit ONCE
    # per session (control-epoch DMD, see build_session_features above) --
    # there is no per-trial v* VECTOR at this call site to average, so:
    # collapse the existing per-condition scalar `vstar_alignment` to its
    # session mean and broadcast back to every row in that session. This is
    # the fixed, pre-computable target a clinician could use without
    # re-estimating v* every trial --
    ...
    modifiers = {
        ...
        # Subspace arms + controls, same call site/tuple as the vstar/amplification columns above.
        "align_s_m2": _col("align_s_m2"),
        "align_s_m3": _col("align_s_m3"),
        "topm_pca_alignment_m2": _col("topm_pca_alignment_m2"),
        "topm_pca_alignment_m3": _col("topm_pca_alignment_m3"),
        "channel_random_alignment": _col("channel_random_alignment"),
    }
    ...
    print(f"\nM1-vs-gate check: gate slope={gate['slope']:.4f}; benchmark M1 "
          ...
          f"({'MATCH' if m1_matches_gate else 'MISMATCH -- reporting as-is'})")
    ...
    with open(RESULTS / "causal_benchmark.json", "w") as f:
        json.dump(_json_safe(bench_json), f, indent=2, allow_nan=False)
    np.savez_compressed(
    ...
    if len(dvs) >= 4:
        ...
        res5 = paired_sign_flip_test(abs_dyn, abs_stab, n_perm=10000, alternative="greater",
                                     rng=np.random.default_rng(stable_seed("macaque_pfc_microstimulation_wp5")))
    ...
    np.savez_compressed(
        RESULTS / "causal_macaque_pfc_microstimulation_gate_detail.npz",
    ...
    with open(RESULTS / "causal_macaque_pfc_microstimulation.json", "w") as f:
        json.dump(_json_safe(out), f, indent=2, allow_nan=False)

    with locked_json_update(RESULTS / "all_statistics.json") as stats:
        stats["causal_macaque_pfc_microstimulation"] = out
        stats["causal_benchmark"] = bench_json
    print("\nSaved results/causal_macaque_pfc_microstimulation.json, results/causal_macaque_pfc_microstimulation_gate_detail.npz, "
