#!/usr/bin/env python3
"""Round-8 Part 9+10 (comments.txt): reconstruct results/causal_benchmark.json
apples-to-apples, THEN promote the correctly-designed arena to primary (K4).

K3: the original "winner" field took argmax(slope) over a MIXED list -- 8 arms
scored on Soldado's causal-stim design (n=15670) and 2 (macrosignal_pac,
rl_policy_alignment) scored on Boran's targeting side-benchmark (n<=6). Not a
valid ranking (K2).

Part 9 fixed the mixing but picked Boran-iEEG as primary_leaderboard because it
fit all 10 arms -- and came back undecidable (n<=6, winner=null, every DML p
flagged untrustworthy, arms scored on DIFFERENT subject subsets so not even
internally apples-to-apples). K4 (PI ruling) supersedes that choice: per K2,
"raw arm-count is not the criterion" -- an underpowered, internally-unequal-n
arena is DESCRIPTIVE BREADTH, not primary, no matter how many arms it fits.
Soldado co-scores 6/10 arms on one identical, adequately-powered, causally-
designed tuple (trial-level DML, n=15670, delivered-uStim) -- that is primary.

Round-8.1 Part 11: two more arms (anat_avg_ctrl, anat_modal_ctrl) were dropped
from the SCORED set -- they are a single area-level structural-controllability
scalar broadcast identically to every session/trial (zero within-arena
variance), not a per-trial steering direction. Regressing the DML pseudo-
outcome on a constant gave a fake slope==0/CI==[0,0]/p==1.0 that read as a
"trustworthy null" rather than a structurally-undefined competitor. They are
now filed eligible=False with a concrete reason, not scored/ranked. Primary is
Soldado-6, not Soldado-8.

This script does NOT re-run any analysis; it only re-reads what
run_soldado_pipeline.py (Step A), run_targeting_benchmark.py, and
run_rl_policy_arm.py already wrote, and writes THREE distinct objects:

  primary_leaderboard        : the SOLDADO-6 causal arena (K4). Ranked by
...
Run (after run_targeting_benchmark.py and run_rl_policy_arm.py):
    conda run -n wm_dynamics python scripts/build_apples_to_apples_leaderboard.py
"""
...
def build_breadth_arena() -> dict:
    """Boran-iEEG, all 10 arms co-eligible. K4: DEMOTED from primary to
    ...
    per-arm destabilization exclusion (not internally apples-to-apples). Its
    value is breadth: the only arena where PAC + RL are computable, and the
    destabilization pattern itself is a real finding (Part 10B)."""
    ...
    breadth = {
        ...
        "role_note": ("K4 (PI ruling 2026-07-12): demoted from primary_leaderboard. Reasons: n<=6 "
                     ...
                     "internally apples-to-apples). Its value is BREADTH: the only arena where PAC and RL "
...
SOLDADO_ANAT_INELIGIBLE_REASON = (
    "area-level structural-controllability scalar, identical for every session (see "
    "run_soldado_pipeline.py's own print: 'same for every session') -- zero within-arena "
    "variance, not a per-trial steering direction, so it cannot be scored as a rankable "
    "competitor in a trial-level design"
)


def build_soldado_primary_leaderboard() -> dict:
    """K4/Part 10A (Round-8.1 Part 11: corrected to Soldado-6). The primary
    leaderboard. Soldado co-scores 6/10 arms on one identical, adequately-
    powered, causally-designed tuple (trial-level DML, n=15670, delivered-
    uStim). macrosignal_pac and rl_policy_alignment are genuinely ineligible
    ...
    for arm in scoreable_arms:
        if arm not in soldado:
            ...
            reason = soldado_excluded.get(arm, {}).get(
                "reason", f"{arm} not present in run_soldado_pipeline.py's leaderboard output")
    ...
    arms["anat_avg_ctrl"] = {"eligible": False, "reason": SOLDADO_ANAT_INELIGIBLE_REASON}
    arms["anat_modal_ctrl"] = {"eligible": False, "reason": SOLDADO_ANAT_INELIGIBLE_REASON}
    arms["macrosignal_pac"] = {
        "eligible": False,
        "reason": ("Soldado is spike-rate data only (run_soldado_pipeline.py loads binned spike rate, "
    ...
    arms["rl_policy_alignment"] = {
        ...
        "reason": ("RL's convergence-check construction (train against a fitted closed-loop simulator, "
                  ...
                  "simulate_closed_loop) has no analogue on Soldado's design: Soldado is a per-trial "
                  ...
                  "simulate_closed_loop harness fit to Soldado's session structure. Building both would be "
                  "a new pipeline component (new simulator + a redefinition of 'donor'), not a cheap "
                  "re-run this round -- concrete scope gap, future work, not faked."),
    }
    ...
    if winner is not None:
        winner_note = (
            f"{winner} ranks first by gate-slope (slope={scored[winner]['slope']:.4f}, "
            f"p={scored[winner]['p_value']:.4g}) on the strongest-design, adequately-powered arena in the "
            "study (n=15670, delivered-uStim causal design, 6 arms co-scored on one identical tuple); "
            f"this is also the only arm whose effect survives session-clustered inference "
            f"(cluster-robust slope={cluster_robust['vstar_alignment']['cluster_robust']['slope']:.4f}, "
            f"p={cluster_robust['vstar_alignment']['cluster_robust']['p_value']:.3g}, "
            "positive in 2/2 monkeys; see soldado_headline_robustness.json)."
        )
        other_sig = [a for a in significant if a != winner]
        if other_sig:
            parts = ", ".join(f"{a} (slope={scored[a]['slope']:.4f}, p={scored[a]['p_value']:.4g})"
                              for a in other_sig)
            winner_note += (f" {parts} {'is' if len(other_sig) == 1 else 'are'} a trial-level second "
                            "significant arm that does not survive session-clustering (min_energy_dir_alignment "
                            "cluster-robust p="
                            f"{cluster_robust['min_energy_dir_alignment']['cluster_robust']['p_value']:.3g}, "
                            "positive in only 4/11 sessions, significant in 1/2 monkeys) -- we therefore do NOT "
                            "count it as an established second arm; report the full ranking, do not cherry-pick "
                            "the top arm alone.")
    ...
    primary = {
        ...
        "exclusion_rule": ("shared trial-level design from run_soldado_pipeline.py (PCA+DMD plant fit on "
        ...
        "cluster_robust_source": "results/soldado_headline_robustness.json (cluster_robust)",
        "winner": winner,
...
def _load_soldado_leaderboard() -> dict:
    """Source of truth for the Soldado-scored (n=15670) arms. Round-8.1 Part 11:
    anat_avg_ctrl/anat_modal_ctrl are no longer passed to benchmark_modifiers
    (degenerate constant), so this dict now has 6 keys, not 8. Read from
    all_statistics.json['causal_benchmark'] (written once by run_soldado_
...
def build_cross_dataset_replication(breadth: dict) -> dict:
    ...
    primary -- `breadth` supplies the Boran-iEEG cells (now the secondary
    breadth arena; see K4)."""
    soldado_arms = {k: v for k, v in _load_soldado_leaderboard().items() if k in DYNAMICS_ARMS}
    ...
    for arm in DYNAMICS_ARMS:
        cell = {}
        if arm in soldado_arms:
            sv = soldado_arms[arm]
            cell["soldado"] = {
                "dataset": "soldado_macaque_ustim", "n": sv["n"], "unit_level": "trial (DML, causal design)",
    ...
    pac_cell["soldado"] = {"eligible": False, "reason": "spike-rate data only (run_soldado_pipeline.py loads binned spikerate, no LFP/broadband channel) -- no continuous phase signal exists to compute PAC from"}
    pac_cell["miller_ecog"] = {"eligible": False, "reason": ("continuous LFP AND a TES1 B-bank both exist (results/tes1_comprehensive.npz has full al/ca/cc/ug bundles) -- structurally eligible -- but Miller's NWB files carry task condition, not response accuracy (see run_closed_loop_behavior_flip.py header), so the outcome-decoder/flip-rate half of the targeting-benchmark construction is not buildable; a drift-reduction-only PAC replication cell (paralleling the Miller vstar_alignment cell above) was NOT built this round -- concrete gap, flagged as future work, not silently skipped")}
    ...
    rl_cell["other_datasets"] = {"eligible": False, "reason": "RL policy training was only run against the Boran A/B plant this round (Part 5 scope); not replicated on Soldado/Miller/Rutishauser plants -- concrete gap, future work"}
...
def self_check(primary: dict, breadth: dict, replication: dict) -> None:
    """Part 10D (+ Round-8.1 Part 11D). primary_leaderboard must be Soldado,
    single dataset/metric/unit-level/n, and every eligible arm in it must be
    p_value_trustworthy AND have a non-degenerate (nonzero-width) CI. The
    breadth arena must never be named/rolled as primary. n_arms_scored must be
    6 (vstar_alignment, min_energy_dir_alignment, random_alignment,
    gramian_trace, input_norm, stable_alignment) plus 1 more (Round-10 Part
    15A: session_mean_vstar_scalar) UNLESS that 7th arm was excluded by
    benchmark_modifiers' zero-variance guard, in which case it stays 6 --
    either way is a legitimate, non-fabricated count, not a hardcoded magic
    number."""
    assert primary["arena"] == "soldado_macaque_ustim", "primary_leaderboard must be the Soldado arena (K4)"
    ...
    assert primary.get("n_arms_scored") == expected_n_arms, \
        (f"primary_leaderboard must score exactly {expected_n_arms} Soldado arms given "
         f"session_mean_vstar_scalar eligible={session_mean_eligible}, "
         f"amplification_alignment eligible={amp_eligible}")
    assert primary.get("cluster_robust_significant_arms") == ["vstar_alignment"], \
        "Round-9 Part 13B: only vstar_alignment survives session-clustering"
    assert "min_energy_dir_alignment" not in primary["cluster_robust_significant_arms"], \
        "min_energy_dir_alignment must not be reported as a cluster-robust significant arm (p=0.40)"
    ...
    print("Self-check passed: primary_leaderboard is single-arena (Soldado, all trustworthy); "
          "breadth arena not aliased as primary; every replication cell independently tagged.")


def main():
    primary = build_soldado_primary_leaderboard()
    ...
    # Round-11 Part 17B: amplification-reframe mechanism block. Built from
    # results/amplification_check.json (17A: cos(v*, w1) per session) and
    # results/amplification_robustness.json (17B: cluster-robust vstar vs
    # amplification_alignment, apples-to-apples, B=2000 session bootstrap) --
    # both produced by their own scripts this round, read back here, never
    # recomputed or hand-edited.
    ...
    new_bench = {
        ...
        "_deprecated_mixed_winner_removed": ("the original top-level 'winner' field (argmax(slope) "
                                             "across ALL 10 arms regardless of dataset/n) violated K2: "
                                             "it returned 'macrosignal_pac' (n=6, Boran) purely because "
                                             "its raw slope magnitude (48.85) numerically exceeded "
                                             "vstar_alignment's Soldado slope (0.033), despite the two "
                                             "being on incomparable scales/designs."),
        "_deprecated_boran_primary_removed": ("Part 9's first cut made Boran-iEEG primary_leaderboard "
                                              "because it fit all 10 arms; K4 (PI ruling) supersedes this: "
                                              "raw arm-count is not the criterion (K2) and Boran-10 was "
                                              "undecidable (n<=6, winner=null, every p untrustworthy, not "
                                              "even internally apples-to-apples). See breadth_descriptive_arena."),
        "_deprecated_anat_arms_as_null_removed": ("Round-8.1 Part 11: anat_avg_ctrl/anat_modal_ctrl were "
                                                  ...
                                                  "Soldado-6, not Soldado-8. See src/causal.py benchmark_"
    ...
    print("Rewrote results/causal_benchmark.json: primary_leaderboard (Soldado-6) + "
          "breadth_descriptive_arena (Boran-10) + cross_dataset_replication.")
    print(f"Primary-arena (Soldado) winner: {primary['winner']}")
