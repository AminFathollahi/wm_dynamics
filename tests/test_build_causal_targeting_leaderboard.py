"""Self-check for scripts/build_causal_targeting_leaderboard.py's pure logic:
the DML-availability formatter and the K2/K4 self-check (macaque PFC microstimulation-arena
...
from build_causal_targeting_leaderboard import (
...
def _valid_primary():
    return {
        ...
        "arms": {
            "vstar_alignment": {
                "eligible": True, "n": 5880, "p_value_trustworthy": True,
...
def test_self_check_passes_on_valid_structure():
    replication = {"vstar_alignment": {"macaque_pfc_microstimulation": {"dataset": "macaque_pfc_microstimulation", "n": 100}}}
    self_check(_valid_primary(), _valid_breadth(), replication)  # must not raise


def test_self_check_rejects_cell_missing_dataset_tag():
    replication = {"vstar_alignment": {"macaque_pfc_microstimulation": {"n": 100}}}  # missing "dataset" key
    with pytest.raises(AssertionError):
        self_check(_valid_primary(), _valid_breadth(), replication)


def test_self_check_rejects_non_macaque_pfc_microstimulation_primary():
...
def test_self_check_rejects_degenerate_zero_width_ci():
    # Catches the anat_avg_ctrl/anat_modal_ctrl bug class -- an eligible arm
    # with slope_ci_lo == slope_ci_hi (e.g. a constant modifier silently
    # regressed to a fake slope=0/CI=[0,0]) must be rejected.
    bad_primary = _valid_primary()
...
def test_self_check_rejects_any_cluster_robust_significant_arm():
    # Neither vstar_alignment nor min_energy_dir_alignment survives session-clustered
    # bootstrap inference (macaque_pfc_microstimulation_headline_robustness.json) -- the primary leaderboard
    # must report an empty cluster_robust_significant_arms list.
    bad_primary = _valid_primary()
    bad_primary["cluster_robust_significant_arms"] = ["vstar_alignment"]
    with pytest.raises(AssertionError):
        self_check(bad_primary, _valid_breadth(), {})
...
if __name__ == "__main__":
    ...
    test_self_check_rejects_non_macaque_pfc_microstimulation_primary()
    ...
    test_self_check_rejects_any_cluster_robust_significant_arm()
